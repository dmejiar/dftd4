...
subroutine get_rational_damping(functional, param, s9)
   ...
   mbd = .true.
   if (present(s9)) mbd = s9.ne.0.0_wp
...
subroutine get_d4eeq_bj_parameter(dfnum, param, s9)
   ...
   select case(dfnum)
   case(p_dftb_3ob)
      allocate(param, source = dftd_param( & ! (SAW191202)
...
contains
   ...
   function dftd_param(s6, s8, a1, a2, alp) result(param)
      ...
      real(wp) :: ss6=1.0_wp, ss9=0.0_wp, aalp=16.0_wp
      if (present(s6)) ss6=s6
      if (present(s9)) ss9=s9
      if (present(alp)) aalp=alp
      param = rational_damping_param(ss6,s8,ss9,a1,a2,aalp)
...
subroutine get_d4eeq_bjatm_parameter(dfnum, param, s9)
   ...
   case(p_lh20t)
      allocate(param, source = dftd_param ( & ! (10.1021/acs.jctc.0c00498)
   ...
   case(p_mn12sx)
      allocate(param, source = dftd_param ( & ! (SAW211021)
   ...
   case(p_rscan)
      allocate(param, source = dftd_param ( & ! (10.1063/5.0041008)
   ...
   case(p_r2scanh)
      allocate(param, source = dftd_param (s6=1.0_wp, s8=0.8324_wp, a1=0.4944_wp, a2=5.9019_wp))
   case(p_r2scan0)
      allocate(param, source = dftd_param (s6=1.0_wp, s8=0.8992_wp, a1=0.4778_wp, a2=5.8779_wp))
   case(p_r2scan50)
      allocate(param, source = dftd_param (s6=1.0_wp, s8=1.0471_wp, a1=0.4574_wp, a2=5.8969_wp))
   case(p_tpss0)
      allocate(param, source = dftd_param ( & ! (SAW190103)
   ...
   case(p_b97d)
      allocate(param, source = dftd_param ( & ! (SAW201029)
   ...
   case(p_b97m)
      allocate(param, source = dftd_param ( & ! (10.1002/jcc.26411)
   ...
   case(p_revdsdpbep86)
      allocate(param, source = dftd_param ( & ! (WTMAD2)
   ...
   case(p_hse03)
      allocate(param, source = dftd_param( & ! (SAW211107)
...
contains
   ...
   function dftd_param(s6, s8, a1, a2, alp) result(param)
      ...
      real(wp), intent(in), optional :: s6, alp
      type(rational_damping_param) :: param
      real(wp) :: ss6=1.0_wp, ss9=1.0_wp, aalp=16.0_wp
      if (present(s6)) ss6=s6
      if (present(s9)) ss9=s9
      if (present(alp)) aalp=alp
      param = rational_damping_param(ss6,s8,ss9,a1,a2,aalp)
